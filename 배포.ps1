...
try {
  ...
  if ($ver -and $rows.Count -gt 0) {
    $seed = @($rows | ForEach-Object { [ordered]@{ id = $_.id; n = $_.number; name = $_.name; cat = $_.category; org = $_.organization; zone = $_.zone; desc = $_.description; video = $_.video_url; pdf = $_.pdf_url; img = $_.image_url; hours = $_.operating_hours; variant = $_.time_variant; target = $_.target_audience } })
