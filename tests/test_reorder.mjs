...
const order = () => p.evaluate(() => $$('#bList .brow').map(tr => $('[data-f=n]', tr).value));
check('화살표 없음', await p.evaluate(() => !$('[data-up]') && !$('[data-down]') && !!$('#bSort')));
const before = await order(); console.log('  처음', before.slice(0, 5).join(','));
// 3번째 행을 맨 위로 끌기
const src3 = await p.locator('.brow[data-i="2"] [data-drag]').boundingBox(); const dst = await p.locator('.brow[data-i="0"]').boundingBox();
...
check('행의 data-i 다시 0부터', await p.evaluate(() => $$('#bList .brow').every((tr, i) => +tr.dataset.i === i)));
// 다른 셀 편집 유지 + 아래로 끌기
await p.fill('.brow[data-i="1"] [data-f=name]', '편집중'); const s1 = await p.locator('.brow[data-i="1"] [data-drag]').boundingBox(); const d4 = await p.locator('.brow[data-i="4"]').boundingBox();
...
check('2번째 행이 5번째로, 편집값 유지', after2[4] === after[1] && await p.evaluate(() => $('.brow[data-i="4"] [data-f=name]').value === '편집중'));
// 번호순 정렬
